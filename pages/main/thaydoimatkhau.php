<?php
	if(isset($_POST['doimatkhau'])){
		...
		$sql = "SELECT * FROM tbl_taikhoan WHERE email='".$taikhoan."' AND matkhau='".$matkhau_cu."' LIMIT 1";
		$row = mysqli_query($mysqli,$sql);
		$count = mysqli_num_rows($row);
		if($count>0){
			$sql_update = mysqli_query($mysqli,"UPDATE tbl_taikhoan SET matkhau='".$matkhau_moi."'");
