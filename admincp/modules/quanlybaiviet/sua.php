...
?>
 <form method="POST" action="modules/quanlybaiviet/xuly.php?idbaiviet=<?php echo $row['id_baiviet'] ?>" enctype="multipart/form-data">
	  <tr>
	  	<td>Tên bài viết</td>
	  	<td><input type="text" value="<?php echo $row['tenbai'] ?>" name="tenbai"></td>
	  </tr>
	  ...
	  <tr>
	    ...
	    <td>
	    	<select name="danhmucbv">
	    		<?php
	    		$sql_danhmuc = "SELECT * FROM tbl_danhmucbv ORDER BY id_danhmucbv DESC";
	    		$query_danhmuc = mysqli_query($mysqli,$sql_danhmuc);
	    		while($row_danhmuc = mysqli_fetch_array($query_danhmuc)){

	    			if($row_danhmuc['id_danhmucbv']==$row['id_danhmucbv']){
	    		?>
	    		<option selected value="<?php echo $row_danhmuc['id_danhmucbv'] ?>"><?php echo $row_danhmuc['tendanhmucbv'] ?></option>
	    		<?php
	    			}else{
	    		?>
